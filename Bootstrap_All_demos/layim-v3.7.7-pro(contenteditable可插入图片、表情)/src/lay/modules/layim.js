...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<div class="layim-chat-textarea"><div class="cea" contenteditable="true"></div></div>'
  ...
  layui.data.content = function(content) {
    if ((/weui-/.test(content))){ //自定义消息模板-weui图文组合列表
      return content
    }else if ((/layui-/.test(content))){ //自定义消息模板-layui图文组合列表
      return content
    }else{
      //支持的html标签
      var html = function (end) {
        return new RegExp('\\n*\\[' + (end || '') + '(a|img|code|pre|div|span|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h1|h2|h3|h4|h5)([\\s\\S]*?)\\]\\n*', 'g');
      };
  ...
  var sendMessage = function(){
    ...
    //jason调整  将表情转化为 文本
    thatChat.textarea.find(".face-expression").each(function(i,t){
      $(t).replaceWith($(t).attr("data-face"))
    })
     ...
     events.face.index = layer.tips(content, othis, {
        ...
        ,success: function(layero){
          ...
          //  focusInsert(thatChat.textarea[0], 'face' +  this.title + ' ');
            focusInsert(thatChat.textarea[0], '<img class="face-expression" data-face="face'+this.title+'" title="'+this.title+'" src="'+ faces[this.title] +'"/>'  + ' ');
